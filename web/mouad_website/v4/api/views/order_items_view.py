...
def get_item(user_id, order_id):
    ...
    if not order:
        abort(404)
    return(jsonify([item.to_dict() for item in order.order_items]))
...
def delete_item(user_id, order_id, item_id):
    ...
    for item in order.order_items:
        if item.id == item_id:
            product = storage.get_by_id(Product, item.product_id)
...
def post_item(user_id, order_id):
    ...
    instance = OrderItem(order_id=order_id, product_id=product_id)
    instance.save()
    order.total_price += product.price
    storage.save()
